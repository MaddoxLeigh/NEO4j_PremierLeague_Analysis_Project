/*
This query is used for inserting goalie player stats for a specific game.

It works as followed:
It is executed with a list of goalkeeper stats, so first we need to unpack this list via unwind
We then find the goalie's player node, via name. We have no two players with the same name.
We match this with squad and season name, and then create a relation between the goalie player and the match the stat is for.
